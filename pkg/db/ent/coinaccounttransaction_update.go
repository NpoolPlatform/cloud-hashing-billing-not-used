...
func (catu *CoinAccountTransactionUpdate) SetFailHold(b bool) *CoinAccountTransactionUpdate {
	catu.mutation.SetFailHold(b)
	return catu
}

// SetNillableFailHold sets the "fail_hold" field if the given value is not nil.
func (catu *CoinAccountTransactionUpdate) SetNillableFailHold(b *bool) *CoinAccountTransactionUpdate {
	if b != nil {
		catu.SetFailHold(*b)
	}
	return catu
...
func (catu *CoinAccountTransactionUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if value, ok := catu.mutation.FailHold(); ok {
		_spec.Fields.Set = append(_spec.Fields.Set, &sqlgraph.FieldSpec{
			Type:   field.TypeBool,
			Value:  value,
...
// SetFailHold sets the "fail_hold" field.
func (catuo *CoinAccountTransactionUpdateOne) SetFailHold(b bool) *CoinAccountTransactionUpdateOne {
	catuo.mutation.SetFailHold(b)
	return catuo
}
...
func (catuo *CoinAccountTransactionUpdateOne) Save(ctx context.Context) (*CoinAccountTransaction, error) {
	...
	if len(catuo.hooks) == 0 {
		...
		v, err := mut.Mutate(ctx, catuo.mutation)
		if err != nil {
			return nil, err
		}
		nv, ok := v.(*CoinAccountTransaction)
		if !ok {
			return nil, fmt.Errorf("unexpected node type %T returned from CoinAccountTransactionMutation", v)
		}
		node = nv
	}
	return node, err
...
func (catuo *CoinAccountTransactionUpdateOne) sqlSave(ctx context.Context) (_node *CoinAccountTransaction, err error) {
	...
	if value, ok := catuo.mutation.FailHold(); ok {
		_spec.Fields.Set = append(_spec.Fields.Set, &sqlgraph.FieldSpec{
			Type:   field.TypeBool,
			Value:  value,
			Column: coinaccounttransaction.FieldFailHold,
